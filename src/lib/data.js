...
const 줄들 = raw.rows.map((line, i) => {
  ...
  return {
    ...
    특성: raw.teuk[+p[6]],
    상태: raw.sangtae[+p[7]],
    // ★ 그 학과가 **실제로 있는 곳**. 대학 본부의 지역과 다를 수 있다
    //   (성균관대 본교 학과 일부는 수원, 연세대 본교 학과 일부는 인천 연수구).
    //   원자료의 「전남광주」는 make_data.py 가 광주·전남으로 갈라 두었다.
    시도: raw.place[+p[8]][0],
    시군구: raw.place[+p[8]][1],
  }
})
...
const 대학들 = raw.univ.map((u, i) => ({
  ...
  // 지역 = 본부가 있는 시·도(원자료 「학교지역」), 소재지 = 본부 지역 안에서 학과가 가장 많은 시·군·구
